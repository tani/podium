...
You can try Podium in your browser at L<https://markup.deno.dev/>.
...
=head3 WebAPI
  ...
  $ curl -X POST -d `$(cat path/to/file.pod)` https://markup.deno.dev/html
  $ cat path/to/file.pod | curl -X POST -d @- https://markup.deno.dev/latex
