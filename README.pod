...
=item C<PodiumProcessor> parses POD documents into a tree structure.
...
which has three methods for simple customization:

=head3 C<registerSimpleFormmatingCode(self, name, fun)>
...
C<fun> is a function that takes a string and returns a string.
  ...
  backend:registerSimpleFormmatingCode('B', function (text)
